...
void auton(void) {
  ...
  task IntakeController(IntakeControl);  //
...
int main() {
  Piston.set(on);
  ramp = bwrd; // ramp direction starts as 'backward', the next step of enable
               // will cause ramp to move forward
  ...
  while (1) {
    wait(100); // Sleep the task for a short amount of time to prevent wasted
               // resources.
  }
  return 0;
}
